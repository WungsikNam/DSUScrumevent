...
function emptyRoom() {
  return { players: [], state: 'lobby', gameType: null, rankCounter: 0, goTime: null, startedAt: null, signalTime: null, resetAt: null };
}
...
async function checkTimeouts(room) {
  ...
  if (room.state === 'countdown' && room.goTime && now >= room.goTime) {
    ...
    if (!room.resetAt) {
      ...
      room.state = 'lobby';
      room.gameType = null;
      room.players.forEach(p => { p.presses = 0; p.rank = null; p.finishedAt = null; p.reactionTime = null; p.tooEarly = false; });
      room.goTime = null; room.startedAt = null; room.signalTime = null; room.resetAt = null;
      await setRoom(room);
    }
  }
  return room;
...
async function joinRoom({ playerId, name, hostPassword }) {
  ...
  room.players.push({ id: playerId, name: String(name).trim().slice(0, 16), isHost, presses: 0, rank: null, finishedAt: null, reactionTime: null, tooEarly: false });
  await setRoom(room);
  return room;
}
...
async function startGame(playerId) {
  ...
  room.startedAt = null; room.signalTime = null; room.resetAt = null;
  await setRoom(room);
  return room;
}
...
async function startReactionGame(playerId) {
  ...
  room.state = 'playing';
  room.signalTime = Date.now() + 3000 + Math.floor(Math.random() * 4000); // 3~7초 랜덤
  room.goTime = null; room.startedAt = null; room.resetAt = null;
  await setRoom(room);
  return room;
}
...
async function recordReaction({ playerId, reactionTime }) {
  ...
  if (now < room.signalTime || Number(reactionTime) < 0) {
    ...
    const maxAllowed = now - room.signalTime + 1000;
    player.reactionTime = Math.max(0, Math.min(Number(reactionTime) || 0, maxAllowed));
  }
...
function publicRoom(room) {
  return {
    ...
    players: room.players.map(({ id, name, isHost, presses, rank, finishedAt, reactionTime, tooEarly }) => ({
      id, name, isHost, rank,
      ...
      timeTaken: (rank != null && finishedAt && room.startedAt)
        ? ((finishedAt - room.startedAt) / 1000).toFixed(2) : null,
      reactionTime: reactionTime !== null && reactionTime !== undefined ? reactionTime : null,
      tooEarly: !!tooEarly,
    })),
  };
}

module.exports = { getRoom, setRoom, joinRoom, leaveRoom, startGame, recordPress, startReactionGame, recordReaction, checkTimeouts, resetToLobby, publicRoom };
