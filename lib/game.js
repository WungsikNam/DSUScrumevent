...
const TYPING_WORDS = [
  ['dog','cat','win','run','ace','hit','fly','tap','top','key','aim','zap','go','jet','pro'],
  ['quick','score','react','mouse','click','first','burst','flash','tiger','blaze','arrow','storm','sword','brave','craft'],
  ['warrior','blazing','thunder','network','mission','browser','victory','captain','digital','company','dynamic','forward','passion','mystery','sunrise'],
  ['challenge','lightning','developer','champions','adventure','nightmare','superhero','explosive','everything','president','marketing','dangerous','character','chocolate','interview'],
  ['championship','revolutionary','extraordinary','communication','acceleration','professional','unconditional','overwhelming','presentation','international'],
];
const TYPING_MAX_ROUNDS = 5;
const TYPING_ROUND_MS = [7000, 9000, 11000, 13000, 15000]; // level 1~5
  ...
  if (room.state === 'countdown' && room.goTime && now >= room.goTime) {
    ...
    if (tg.nextRoundAt !== null && now >= tg.nextRoundAt) {
      if (tg.round >= tg.maxRounds) {
        ...
        tg.round++;
        const levelPool = TYPING_WORDS[Math.min(tg.round - 1, TYPING_WORDS.length - 1)];
        const available = levelPool.filter(w => !usedPhrases.includes(w));
        const pool = available.length > 0 ? available : levelPool;
        const phrase = pool[Math.floor(Math.random() * pool.length)];
        tg.phrase = phrase; tg.usedPhrases = [...usedPhrases, phrase];
        tg.roundStartTime = now; tg.roundWinner = null; tg.nextRoundAt = null;
        await setRoom(room);
      }
    } else if (tg.nextRoundAt === null && tg.roundStartTime && now > tg.roundStartTime + TYPING_ROUND_MS[Math.min(tg.round - 1, 4)]) {
